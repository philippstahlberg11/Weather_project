    ...
    public void showGridFirst(int initialValue) {
        ...
        for (int row = 1; row < tetrisBoard.rows(); row++) {
            tetrisBoard.set(new CellPosition(row, 0),
                    modellWeather.convertTimeToHours(modellWeather.getNextHoursDetails(tetrisBoard.rows() + (this.value)).get(row-1 + (this.value))));
            ...
            for (int col = 2; col < tetrisBoard.cols(); col++) {
                ...
                tetrisBoard.set(new CellPosition(row, col),
                        modellWeather.getTimeDetails(row-1 + (this.value), modellWeather.genericDetailsInfoList().get(col - 2)));
